...
def analyze_trill(
    ...
    """
    Full analysis of the R trill.

    Args:
        audio_path: Path to the audio file
        frc_cross_time: FRC crossing time (seconds)
        output_folder: Output folder for figures
        show_plots: Display plots
    """
    print(f"\n{'='*60}")
    print("R TRILL ANALYSIS")
    print(f"{'='*60}")
    print(f"File: {audio_path}")
    print(f"FRC crossing: {frc_cross_time or 'not specified'}")
    print(f"{'='*60}\n")
    ...
    print("\n3. Modulation frequency analysis...")
    ...
    if frc_cross_time is not None:
        ...
        mod_full = modulation.compute_modulation_frequency(audio_segment, sr)
        print(f"   Modulation frequency: {mod_full:.1f} Hz")
        frc_segment = None

    # Onset count (proxy for trill cycles)
    onsets = librosa.onset.onset_detect(y=audio_segment, sr=sr)
    print(f"\n   Number of onsets detected: {len(onsets)}")
    print(f"   Frequency estimated from onsets: {len(onsets) / (len(audio_segment)/sr):.1f} Hz")
    
    # Visualization
    print("\n4. Generating figures...")

    # Figure 1: Waveform with segmentation
    ...
    axes1[0].set_title('R Trill - Waveform')
    ...
    axes1[1].set_title('Spectrogram')

    # RMS envelope and modulation
    ...
    axes1[2].set_title(f'RMS Envelope (Modulation freq.: {mod_full:.1f} Hz)')
    
    plt.tight_layout()
    
    # Figure 2: Modulation spectrum
    fig2, ax2 = plt.subplots(figsize=(10, 5))

    # FFT of the RMS envelope
    ...
    print("SUMMARY")
    print(f"{'='*60}")
    print(f"Phonation duration: {end_time - start_time:.2f} s")
    print(f"Mean F0: {acoustic.pitch.mean_f0:.1f} Hz")
    print(f"HNR: {acoustic.voice_quality.hnr:.1f} dB")
    print(f"Modulation frequency: {mod_full:.1f} Hz")
    print(f"{'='*60}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="R trill analysis with modulation frequency"
    )
    parser.add_argument(
        "audio_path",
        type=Path,
        help="Path to the trill audio file"
    )
    parser.add_argument(
        "--frc-time",
        type=float,
        default=None,
        help="FRC crossing time in seconds"
    )
    parser.add_argument(
        "-o", "--output",
        type=Path,
